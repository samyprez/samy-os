                ...
                schema: {
                  ...
                  properties: {
                    operation: {
                      ...
                      description:
                        "IMPORTANTE (2026-08-10): tareas/proyectos y clientes viven SOLO en la oficina virtual (app.amazingsolutions.ca, el Hub) — ya no existe una lista de tareas ni de clientes separada dentro de Samy OS. list_tasks, create_task y complete_task son alias de las operaciones del Hub: list_tasks lista el tablero (filtra por status o query), create_task crea una tarjeta nueva (requiere title; usa name o related_to para el cliente si aplica), complete_task la marca terminada (requiere task_id, o basta el nombre/title si no tienes el id). list_clients, create_client y update_client también son ya los clientes reales del Hub (requiere name para crear; client_id para actualizar, o basta el nombre). list_hub_clients y list_projects/create_project/update_project/add_project_note siguen existiendo como sinónimos exactos de list_clients y list_tasks/create_task/complete_task/add_project_note respectivamente — usa cualquiera de los dos nombres, van al mismo lugar. add_project_note añade una nota al historial de una tarjeta (requiere project y note). list_invoices lista facturas con montos y vencimientos. overview: resumen de pendientes (del Hub), notas personales, clientes (del Hub) y próximos eventos. list_notes: listar notas PERSONALES de Samy (no del Hub). create_note: guardar una nota personal (requiere body). list_events: próximos eventos del calendario personal, o búsqueda si mandas query. create_event: agendar un evento personal (requiere title y starts_at). list_brands: listar marcas. create_brand: registrar una marca (requiere name). search_email: buscar correos en el Gmail de Samy con la sintaxis de Gmail en query (por ejemplo 'from:cliente@correo.com', 'is:unread', 'newer_than:7d'); devuelve remitente, asunto, fecha y un extracto, más el id de cada mensaje. read_email: leer un correo completo (requiere message_id, sacado antes de search_email); úsalo cuando el extracto no alcance para responder.",
                    },
                    title: {
                      type: "string",
                      description:
